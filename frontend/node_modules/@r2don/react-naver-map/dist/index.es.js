...
function Mb$1(a, b) {
  try {
    ...
    for (e = 0; e < f.length; e++) {
      var g = f[e];
      d = b;
      var h = a.responseState, k2 = g.id, m = g.errorDigest, n2 = g.errorMessage, q = g.errorComponentStack;
...
function Sa(a, b, c, d, f) {
  switch (b) {
    ...
    case "input":
      ...
      null !== r2 ? J(a, d, "checked", r2) : null !== m && J(a, d, "checked", m);
    ...
    switch (c) {
      ...
      case ac:
        a: {
          ...
          b.blockedSegment = q;
          try {
            if ($c(a, b, d), q.lastPushedText && q.textEmbedded && q.chunks.push(Ca), q.status = 1, ad(h, q), 0 === h.pendingTasks)
              break a;
          } catch (r2) {
...
function fd(a, b) {
  ...
  try {
    ...
    for (e = 0; e < f.length; e++) {
      var g = f[e];
      d = b;
      var h = a.responseState, m = g.id, q = g.errorDigest, r2 = g.errorMessage, v2 = g.errorComponentStack;
...
const useCustomIcon = (icon, reactIcon) => {
  if (icon && reactIcon)
...
const MarkerBase = ({
  ...
  useImperativeHandle(ref, () => ({
    getMarker: () => ({ marker: marker.current, id: id2 })
  }));
  useIsomorphicLayoutEffect(() => {
...
const Marker = memo(forwardRef(MarkerBase));
const Cluster = ({ children, options }) => {
  const markersRef = useRef({});
  const clusterRef = useRef();
  const map = useMapContext();
  useIsomorphicLayoutEffect(() => {
    import("./MarkerClustering.js").then(({ MarkerClustering: MarkerClustering2 }) => {
      var _a;
      if (clusterRef.current)
        return;
      const icons = (_a = options == null ? void 0 : options.icons) == null ? void 0 : _a.map((icon) => {
      ...
      const cluster = new MarkerClustering2({
        ...options,
        icons,
        map,
        markers: Object.values(markersRef.current).filter((ref) => !!ref.getMarker).map((ref) => ref.getMarker().marker)
      });
    ...
    return () => {
      if (clusterRef.current) {
        ...
        clusterRef.current = void 0;
        markersRef.current = {};
      }
    };
  }, [children, map, options]);
  return /* @__PURE__ */ React.createElement(React.Fragment, null, children.map((child) => cloneElement(child, {
    ref: (ref) => {
      var _a;
      const marker = (_a = ref == null ? void 0 : ref.getMarker) == null ? void 0 : _a.call(ref);
      if (!marker)
        return;
...
const SCRIPT_ID = "react-naver-map";
const createNaverMapScriptByClientId = ({
  ncpClientId,
  submodules
}) => {
  const script = document.createElement("script");
  let paramsString = `ncpClientId=${ncpClientId}`;
  if (submodules == null ? void 0 : submodules.length) {
...
const insertNaverMapScriptIntoHead = (script) => {
  const { head } = document;
...
const useNaverMapInit = ({
  ...
  const [{ isLoaded, isError }, setInitResult] = useState({
    isLoaded: false,
    isError: false
  });
  useIsomorphicLayoutEffect(() => {
    ...
    if (document.getElementById(SCRIPT_ID) && Boolean((_a = window == null ? void 0 : window.naver) == null ? void 0 : _a.maps)) {
    ...
    const initNaverMapScript = async () => {
      const scriptInitResult = new Promise((resolve, reject) => {
        ...
        script.addEventListener("error", function() {
          ...
          (_a2 = document.getElementById(SCRIPT_ID)) == null ? void 0 : _a2.remove();
...
const useCenter = () => {
  ...
  const setCenter = useCallback(({ latitude, longitude }) => {
...
export { Cluster, Map$1 as Map, MapContextProvider, Marker, ZoomControl, useCenter, useMapBounds, useMapContext, useNaverMapInit, useZoom };
